...
class CookieConsent
{
    ...
    {
        $stylePath = 'vendor/w01ki3/cookie-consent/css/style.css';
        if (File::exists(public_path($stylePath))) {
            return '<link rel="stylesheet" href="' . $this->getDynamicAsset($stylePath) . '">';
        }
        return '<link rel="stylesheet" href="' . url('vendor/w01ki3/cookie-consent/assets/css/style.css') . '">';
    }
    ...
    {
        $config = (array)$this->config->get('cookie-consent');
    ...
    {
        $script = $this->scriptTag('vendor/w01ki3/cookie-consent/assets/js/script.js');
        $defaultJsPath = 'vendor/w01ki3/cookie-consent/js/script.js';
        if (File::exists(public_path($defaultJsPath))) {
            $script = $this->scriptTag($defaultJsPath);
        }

        $script .= '<script src="' . route('cookie-consent.script-utils') . '"></script>';
        return $script;
    }
